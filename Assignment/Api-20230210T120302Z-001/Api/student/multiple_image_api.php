	...
	{				
		...
		if(!file_exists($upload_path . $name))
		{
			...
			if($fileSize < 5000000){
				...
				move_uploaded_file($tempPath, $upload_path . $name); // move file from system temporary path to our upload folder path 
                
                //insert into database table
                $query =  mysqli_query($conn,'INSERT into multi_image (name) VALUES("'.$name.'")');
